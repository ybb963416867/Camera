        ...
        if (type == 0) {
            ...
            if (viewWidth > viewHeight) {
                ...
                if (bitmapAspectRatio > viewAspectRatio) {
                    ...
                    Matrix.orthoM(mProjectMatrix, 0, -1f, 1f, bottom, top, 3f, 7f)
                }
            }
        } else if (type == 2) {
            if (viewWidth > viewHeight) {
...
//        Matrix.setLookAtM(mViewMatrix, 0, 0, 0, 7.0f, 0f, 0f, 0f, 0f, 1.0f, 0.0f);
        Log.e(
            "xxx",
            "center x" + (-viewAspectRatio / bitmapAspectRatio + viewAspectRatio / bitmapAspectRatio) / 2f + " y = " + (-1 + 1) / 2 + "  left = " + left + " right = " + right + " bottom " + bottom + " top = " + top
        )

        offsetX = (left + right) / 2f
        offsetX =
            if (offsetX <= 0) (1 + offsetX) - surfaceWidth / surfaceHeight.toFloat() else surfaceWidth / surfaceHeight.toFloat() - offsetX - 1
        //计算变换矩阵
        Matrix.multiplyMM(matrix, 0, mProjectMatrix, 0, mViewMatrix, 0)
        Matrix.translateM(
            matrix,
            0,
            (right - left) / 2f * 1.6666f - 1.6666f,
            (bottom + top) / 2f,
            0f
        )
